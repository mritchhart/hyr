(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('hopeRanchLearningAcademyApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-hopeRanchLearningAcademyApp-error');
                var entityKey = httpResponse.headers('X-hopeRanchLearningAcademyApp-params');
                if (errorHeader) {
